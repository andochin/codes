...
int main()
{
  ...
  point_of_throw_type   point_of_throw(21, 0);
  game_type             frame_data(10, frame_type(18, None));
  int                   frame_index = 0;
  size_t                throw_in_game = 0;
  bool                  next_frame = false;
  bool                  game_over = false;

  frame_data[frame_index].first = 0;
  frame_data[frame_index].second = Normal;
  do {
    frame_type& current_frame = frame_data[frame_index];
    size_t throw_in_frame = throw_in_game - current_frame.first;
    // Input throw point
    char point;
    std::cin >> point;
    point = toupper(point); // Convert to upper case1
    point = (point == 'G') ? '0' : point;
    string::size_type pos = valid_chars.find(point);
    ...
    if(current_frame.second == Last) {
      ...
      if(point == 'S') {
        ...
        int point_of_frame = point_of_throw[current_frame.first] + pos;
    ...
    if(next_frame) {
      ++frame_index;
      frame_data[frame_index].first = throw_in_game + 1;
      frame_data[frame_index].second = (frame_index == (frames_in_game - 1)) ? Last : Normal;
      next_frame = false;
    }
